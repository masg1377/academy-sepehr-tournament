  ...
  for (const child of children) {
    ...
    if (
      ...
      (child.navLink === currentUrl ||
        (currentUrl && currentUrl.includes(child.navLink)))
    ) {
      return true;
    }
  }
...
export const removeChildren = (
  ...
  children.forEach((child: any) => {
    if (currentActiveGroup && !currentActiveGroup.includes(child.id)) {
...
const checkForVisibleChild = (arr: any, ability: any) => {
  return arr.some((i: any) => {
    if (i.children) {
      ...
      return ability && ability.can(i.action, i.resource);
    }
  });
};
...
export const canViewMenuItem = (item: any) => {
  const ability = useContext(AbilityContext);
  return ability && ability.can(item.action, item.resource);
};
