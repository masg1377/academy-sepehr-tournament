...
interface INavbarBookmarksProp {
  setMenuVisibility: any;
}

const NavbarBookmarks: FC<INavbarBookmarksProp> = ({
  setMenuVisibility,
}): JSX.Element => {
  // ** State
  ...
  const store: any = useSelector((state: RootState) => state.navbar);
  ...
  const renderBookmarks = (): JSX.Element | null => {
  ...
  const renderExtraBookmarksDropdown = (): JSX.Element | null => {
  ...
  const handleBookmarkVisibility = (): void => {
  ...
  const handleClearInput = (setUserInput: (val: string) => void) => {
    if (!openSearch) {
      setUserInput("");
      handleClearQueryInStore();
    }
